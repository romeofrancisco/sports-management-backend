...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            type=int,
            default=8,
            help="Number of random teams to create (default 8)",
        )
        parser.add_argument(
            "--sport", type=str, help="Sport slug to use for created teams"
        )
        parser.add_argument(
            "--division",
            type=str,
            choices=["male", "female"],
            help="Force division for created teams",
        )
        parser.add_argument(
            "--head-coach-email",
            type=str,
            help="Email of head coach to assign to all teams (optional)",
        )
        parser.add_argument(
            "--assistant-coach-email",
            type=str,
            help="Email of assistant coach to assign to all teams (optional)",
        )
        parser.add_argument(
            "--create-head-coach",
            action="store_true",
            help="Create a random head coach account for each created team",
        )
    ...
    def handle(self, *args, **options):
        ...
        for i in range(1, count + 1):
            ...
            if Team.objects.filter(
                name__iexact=name, sport=sport, division=team_div
            ).exists():
                name = f"{name} {i}"
            abbreviation = "".join(word[0] for word in name.split())[:3].upper() + str(
                i
            )
                ...
                with transaction.atomic():
                    ...
                    if not hc_to_use and create_head_coach_flag:
                        ...
                        while User.objects.filter(email=email).exists():
                            ...
                            c += 1
                        try:
                            user = User.objects.create_coach(
                                email=email,
                                first_name=first,
                                last_name=last,
                                sex=sex,
                    ...
                    sys.stdout.write(
                        f"[{i}/{count}] Created team: {name} ({sport.name} / {team_div})\n"
                    )
                    sys.stdout.flush()
