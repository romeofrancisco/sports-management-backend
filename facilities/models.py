...
class Reservation(models.Model):
    ...
    coach = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="coach_reservations"
    )
    ...
    def clean(self):
        ...
        if self.start_datetime < now:
            raise ValidationError(
                {"start_datetime": "Start time cannot be in the past"}
            )
    ...
    def save(self, *args, **kwargs):
        ...
        if self.status == Reservation.Status.APPROVED:
            ...
            Reservation.objects.filter(overlap_q).exclude(pk=self.pk).filter(
                status=Reservation.Status.PENDING
            ).update(status=Reservation.Status.REJECTED)

    def __str__(self):
        return f"Reservation {self.id} - {self.facility.name} ({self.start_datetime} -> {self.end_datetime})"

    def auto_expire(self):
        """Automatically expire pending reservations whose start time has passed."""
        if (
            self.status == Reservation.Status.PENDING
            and self.start_datetime < timezone.now()
        ):
            self.status = Reservation.Status.EXPIRED
