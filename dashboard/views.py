...
from .serializers import (
    AdminOverviewSerializer, AdminAnalyticsSerializer,
    CoachOverviewSerializer, CoachPlayerProgressSerializer,
    PlayerOverviewSerializer, PlayerProgressDetailSerializer
)
...
class DashboardViewSet(viewsets.ViewSet):
    ...
    def admin_overview(self, request):
        ...
        total_players = Player.objects.filter(team__isnull=False).count()  # Only count players with teams
        ...
        recent_training_sessions = TrainingSession.objects.filter(date__gte=last_30_days.date()).count()
        
        # Team distribution by sport
        teams_by_sport = Team.objects.values('sport__name').annotate(
            count=Count('id')
        ).order_by('-count')
        
        # Player distribution by sport
        players_by_sport = Player.objects.filter(
            team__isnull=False,  # Only include players with teams
            team__sport__isnull=False  # Only include teams with sports
        ).values('team__sport__name').annotate(
            count=Count('user')
        ).order_by('-count')
        ...
        # Games by status
        games_by_status = Game.objects.values('status').annotate(
            count=Count('id')
        )
    ...
    def admin_analytics(self, request):
        ...
        if total_training_records > 0:
            present_count = PlayerTraining.objects.filter(attendance_status='present').count()
    ...
    def coach_overview(self, request):
        ...
        try:
            if not hasattr(user, 'coach_profile'):
                return Response({'error': 'Coach profile not found'}, status=status.HTTP_400_BAD_REQUEST)
            ...
            for team in coach_teams:
                ...
                present_records = team_training_records.filter(attendance_status='present').count()
                ...
                team_attendance_data.append({
                    ...
                    'total_sessions': team_training_records.values('session').distinct().count(),
            ...
            recent_sessions_data = [{
                ...
                'attendance_count': session.player_records.filter(attendance_status='present').count(),
                'total_players': session.player_records.count()            } for session in recent_sessions]
    ...
    def coach_player_progress(self, request):
        ...
        coach_teams = user.coach_profile.teams.all()
        players = Player.objects.filter(team__in=coach_teams)
        ...
        for player in players[:15]:  # Limit to top 15 for dashboard
            ...
            attended_sessions = recent_training_records.filter(attendance_status='present').count()
            ...
            player_progress_data.append({
                ...
                'last_training_date': recent_training_records.order_by('-session__date').first().session.date if recent_training_records.exists() else None            })
        
        data = {
            'player_progress': player_progress_data
        }
    ...
    def player_overview(self, request):
        ...
        try:
            if not hasattr(user, 'player_profile'):
                return Response({'error': 'Player profile not found'}, status=status.HTTP_400_BAD_REQUEST)
            ...
            attended_sessions = player_training_records.filter(attendance_status='present').count()
            attendance_rate = (attended_sessions / total_sessions * 100) if total_sessions > 0 else 0
            ...
            if player.team:
                team_info = {
                    ...
                    'coach': player.team.coach.user.get_full_name() if player.team.coach else 'No Coach'
                }
              # Player positions
    ...
    def player_progress(self, request):
        ...
        for period_name, start_date in time_periods.items():
            ...
            progress_data[period_name] = {
                ...
                'training_sessions_attended': training_sessions.filter(attendance_status='present').count(),
                'total_training_sessions': training_sessions.count()
            }
          # Get metric trends (last 10 records for each metric)
        metric_trends = {}
        unique_metrics = PlayerMetricRecord.objects.filter(
            player_training__player=player
        ).values_list('metric__name', flat=True).distinct()
