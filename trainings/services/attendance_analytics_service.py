...
class AttendanceAnalyticsService:
    ...
    def calculate_attendance_overview(base_queryset, filters):
        ...
        overall_rate = (
            (attended_count_actual / total_records * 100) if total_records > 0 else 0
        )

        # Average attended players per session (actual count per session)
        avg_attended_per_session = (
            attended_count_actual / total_sessions if total_sessions > 0 else 0
        )
    ...
    @staticmethod
    def calculate_attendance_trends(base_queryset, filters, period="monthly"):
        """
        Calculate training sessions trends over time
        ...
        Returns:
            list: Trend data with session counts based on period
        """
        ...
        if period == "monthly":
            # Group by month and count unique sessions
            time_series = (
                attendance_qs.annotate(time_period=TruncMonth("session__date"))
                .values("time_period")
                .annotate(
                    session_count=Count("session", distinct=True),
                    total_attendance_records=Count("id"),
                    present_count=Count("id", filter=Q(attendance_status="present")),
            ...
            for period_data in time_series:
                ...
                total_records = period_data["total_attendance_records"]
                present = period_data["present_count"]
                late = period_data["late_count"]
                attended = present + late

                # Calculate average attendance rate for the month
                attendance_rate = (
                    (attended / total_records * 100) if total_records > 0 else 0
                )

                trends_data.append(
                    {
                        "month": time_period.strftime("%Y-%m"),
            ...
            # Group by week for backwards compatibility
            time_series = (
                attendance_qs.annotate(time_period=TruncWeek("session__date"))
                .values("time_period")
                .annotate(
                    session_count=Count("session", distinct=True),
                    total_attendance_records=Count("id"),
            ...
            # Daily period for backwards compatibility
            session_dates = (
                attendance_qs.values_list("session__date", flat=True)
                .distinct()
        ...
        for record in attendance_qs:
            ...
            if player_id not in player_stats:
                player_stats[player_id] = {
                    ...
                    "player_profile": (
                        request.build_absolute_uri(record.player.user.profile.url)
                        if record.player.user.profile and request
                        else None
                    ),
                    "total_sessions": 0,
                    "present_count": 0,
    ...
    @staticmethod
    def get_player_detail_analytics(
        player_id, base_queryset, filters, user=None, request=None
    ):
        """
        ...
        attendance_qs = (
            ...
            .select_related("player__user", "session")
            .order_by("session__date")
        )
        ...
        if user and not user.is_admin:
            if hasattr(user, "player_profile"):
                ...
                player_accessible = attendance_qs.filter(
                    session__team__in=coach_teams
                ).exists()
                if not player_accessible:
                    raise PermissionDenied(
                        "You can only view attendance data for players in your teams"
                    )
        ...
        attendance_rate = (
            (attended_count / total_sessions * 100) if total_sessions > 0 else 0
        )
        ...
        recent_records = attendance_qs.order_by("-session__date")
        trends = []
        ...
        return {
            ...
            "player_profile": (
                request.build_absolute_uri(player_profile.user.profile.url)
                if player_profile.user.profile and request
                else None
            ),
            "total_sessions": total_sessions,
            "attendance_rate": round(attendance_rate, 2),
...
class TeamAnalyticsService:
    ...
    def _calculate_team_activity_metrics(attendance_data, training_sessions):
        ...
        sessions_per_week = (
            (total_sessions / 4.3) if total_sessions > 0 else 0
        )  # 4.3 weeks in month
    ...
    def _calculate_participation_insights(attendance_data):
        ...
        present_ratio = (
            distribution.get("present", 0) / total_records if total_records > 0 else 0
        )
        late_ratio = (
            distribution.get("late", 0) / total_records if total_records > 0 else 0
        )
    ...
    @staticmethod
    def _calculate_overall_summary_score(
        performance_metrics, activity_metrics, participation_insights
    ):
        ...
        activity_score = min(
            100, (sessions_per_week / 3) * 100
        )  # 3 sessions per week = 100%
        ...
        engagement_score = engagement_mapping.get(
            participation_insights["engagement_level"], 50
        )
...
class TrainingEfficiencyService:
    ...
    @staticmethod
    def calculate_training_effectiveness_score(
        team_slug, training_data, attendance_data
    ):
        """
        ...
        if attendance_data.get("attendance_distribution"):
            ...
            consistent_attendance = (
                dist.get("present", 0) + dist.get("late", 0)
            ) / total_records
        ...
        return [
            {
                ...
                "attendance_rate": (
                    round(sum(data["attendance_rates"]) / len(data["attendance_rates"]))
                    if data["attendance_rates"]
                    else 0
                ),
                "avg_duration": (
                    round(data["total_duration"] / data["count"])
                    if data["count"] > 0
                    else 0
                ),
