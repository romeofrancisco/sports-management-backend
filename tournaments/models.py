...
class Tournament(models.Model):
    ...
    def complete_tournament(self):
        ...
        if not bracket:
            raise ValidationError(
                "Cannot complete tournament without a bracket. Generate and complete the bracket first."
            )

        if not bracket.is_complete:
            raise ValidationError(
                "Cannot complete tournament because the bracket is not completed yet."
            )
        ...
        if remaining_games > 0:
            raise ValidationError(
                f"Cannot complete tournament because {remaining_games} game(s) are still not finished."
            )
