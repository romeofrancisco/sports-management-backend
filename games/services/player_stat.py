...
class PlayerStatsSummaryService:
    def __init__(self, game_id, team_filter=None):
        ...
        self.counter_abbrevs = set(
            self.all_stats.filter(is_counter=True, calculation_type="none")
    ...
    def _aggregate_base_stats(self):
        return (
            PlayerStat.objects
            .filter(game=self.game, stat_type__in=self.base_stats, player__team__in=self.teams)
    ...
    def _populate_base(self, summary):
        for rec in self._aggregate_base_stats():
            pid, per, abbr, cnt = (
                rec["player_id"], rec["period"],
                rec["stat_type__abbreviation"], rec["count"]
            )
    ...
    def _compute_pct_composites(self, summary):
        for comp in self.pct_composites:
            ...
            for data in summary.values():
                for pd in data["periods"].values():
                    made = pd["base_stats"].get(made_abbr, 0) + pd["calculated_stats"].get(made_abbr, 0)
                    att  = pd["base_stats"].get(att_abbr, 0)  + pd["calculated_stats"].get(att_abbr, 0)
    ...
    def _build_response(self, summary):
        ...
        for data in summary.values():
            ...
            for per in range(1, self.game.current_period + 1):
                pd = data["periods"][per]
                pts = sum(
                    (pd["base_stats"].get(s.abbreviation, 0) + pd["calculated_stats"].get(s.abbreviation, 0))
                    * s.point_value
                    for s in self.all_stats
                    if s.point_value
                )

                fb = {k: v for k, v in pd["base_stats"].items() if k not in self.counter_abbrevs}
                fc = {k: int(v) if k.endswith("_AT") else v
